...
//Кодировщик, работает с байт-массивом
...
//Декодировщик, работает с байт-массивом 
...
//Обрезка пути, нужно для грамотного переноса подкаталогов
...
//Кодирование файлов в папке, имеет рекурсивный метод просмотра файлов, переносит все файлы и папки
void encrypt_files(vector < string > & paths,
  const string & current_path,
    const string & path_to, string key) {
...
//Декодирование файлов в папке, имеет рекурсивный метод просмотра файлов, переносит все файлы и подкаталоги
void decrypt_files(vector < string > & paths,
  const string & current_path, string output_folder, string key) {
...
//Сканирование в live-режиме для кодирования
...
//Сканирование в live-режиме для декодирования
...
//Главная функция и парсер консольных команд
//Для корректной работы программы необходимо правильно ввести все аргументы и выбрать нужный режим сканирования
int main(int argc, char * argv[]) {
  po::options_description description("Basic commands, please, take all arguments to correcting start:");

  description.add_options()
    ("help,h", "Show help")
    ("mode,m", po::value < string > (), "Select mode e/encrypt or d/decrypt")
  ...
  if (vm.count("mode")) {
    if (vm["mode"].as < string > () == "e"
      or vm["mode"].as < string > () == "encrypt") {
      check_mode = true;
      cout << "The mode is: encryption" << endl;
    } else if (vm["mode"].as < string > () == "d"
      or vm["mode"].as < string > () == "decrypt") {
      check_mode = true;
      cout << "The mode is: decryption" << endl;
    } else {
      cout << "The mode is incorrect, please select  e/encrypt or d/decrypt" << endl;
    }
